import { deniedOp, extractFields, isDestructiveGraphql, requireConfirm } from './denied-ops.mjs';
...
export async function adminGraphql(query, variables, opts = {}) {
  ...
  const channel = process.env.LESUTO_CHANNEL_TOKEN;
  if (!key || !channel) {
    throw new Error('Set LESUTO_AGENT_KEY and LESUTO_CHANNEL_TOKEN (Command Center or Seller → AI Agent Access).');
  }
  ...
  if (isDestructiveGraphql(query)) {
    requireConfirm(opts, 'GraphQL mutation');
  }
